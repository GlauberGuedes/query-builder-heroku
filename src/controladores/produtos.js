...
const listarProdutos = async (req, res) => {
    ...
    try {
        if(categoria) {
            const produtosFiltrados = await knex('produtos').where('categoria', 'ilike', `%${categoria}%`).andWhere('usuario_id', usuario.id).debug();
...
const obterProduto = async (req, res) => {
    ...
    try {
        const produto = await knex('produtos').where({id, usuario_id: usuario.id}).first().debug();
...
const excluirProduto = async (req, res) => {
    ...
    try {
        ...
        .del()
        .where({id, usuario_id: usuario.id}).debug();
